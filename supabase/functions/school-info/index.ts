...
serve(async (req) => {
  ...
  try {
    ...
    const schoolData = {
      ...
      sourceUrl: `https://www.google.com/search?q=${encodeURIComponent(name + ' ' + (country || '') + ' university facts')}`,
    ...
    try {
      ...
      const knownData = knownUniversities[schoolKey] || 
                       knownUniversities[schoolKey.replace('massachusetts institute of technology', 'mit')];

      if (knownData) {
        Object.assign(schoolData, knownData);
        console.log(`Found data for known university: ${name}`);
      } else {
        console.log(`No known data for ${name}, returning search links`);
        // For unknown universities, provide search links
        schoolData.officialUrl = `https://www.google.com/search?q=${encodeURIComponent(name + ' official website')}`;
        schoolData.sourceUrl = searchUrl;
      }
    } catch (searchError) {
